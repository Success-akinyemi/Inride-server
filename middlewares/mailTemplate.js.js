...
export async function sendDriverNotClearedEmail({
  ...
  export async function sendDriverReportCanceledEmail({
    email,
    name = "",
    driverName = "",
    driverId = "",
    ...
    driverReportStatus = "",
    time = Date.now(),
    title = "Checkr Driver Background Check Canceled",
  }) {
    if (!email) {
      throw new Error("Email is required to send a background check canceled email.");
    }
    ...
    const emailContent = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto;">
          ...
          <p style="color: #344054; font-size: 16px; font-weight: 400;">
            <strong>
                Checkr background check for Driver <strong>${driverName}</strong> has been canceled.
            </strong>    
          </p>
          <br />
          <strong>Details:</strong>
